          ...
          {!hasFiles ? (
            ...
            <motion.div variants={sheetItem} className="flex flex-col gap-2">
              {files.map((f, i) => (
                <div
                  key={`${f.name}-${i}`}
                  className="flex items-center gap-3 bg-[var(--bg-neutral-secondary)] border border-[rgba(160,160,160,0.18)] rounded-xl px-4 py-3"
                >
                  <InsertDriveFileOutlinedIcon style={{ fontSize: 22, color: "var(--text-secondary)" }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] leading-[1.3] text-[var(--text-primary)] truncate" style={FONT}>{f.name}</p>
                  ...
                  <button type="button" onClick={() => removeFile(i)} aria-label="Remove" className="shrink-0">
                    <DeleteOutlineIcon style={{ fontSize: 18, color: "var(--text-secondary)" }} />
                  </button>
                </div>
              ))}
            </motion.div>
