...
import { getAccount } from "./ReceivingAccountSheet";
import { FilePreviewOverlay, type UploadedFileInfo } from "./UploadedFile";
import { CREDIT_NOTES } from "./creditNotesData";
...
/** The full invoice lifecycle (DES-715 / DES-716 status matrix). "Cancelled" (DES-719) replaces
 *  the earlier "Void" wording — a fully credited invoice ends in Cancelled. */
...
const STATUS_META: Record<DetailStatus, { label: string; bg: string; border: string; text: string }> = {
  ...
  Cancelled: { label: "Cancelled", bg: "#f3f3f3", border: "rgba(160,160,160,0.35)", text: "#808080" },
...
// Format an ISO date ("2026-06-22") from the manual-refund date input into "22 Jun 2026".
...
export function InvoiceDetailPage({
  ...
  // Each carries its own (possibly edited) client info + credited lines, applied to that credit note only.
  // DES-720: evidence captured when a refund is "marked as already refunded" (done outside Statrys).
  type RefundProof = { date: string; method: string; amount: number; proofFile?: string };
  ...
  type CreditNote = { no: string; amount: number; applied?: number; cancelled?: boolean; name: string; email: string; lines: { name: string; amount: number }[]; date: string; updatedDate?: string; reason?: string; reasonNote?: string; draftLines?: DraftLine[]; issueDate?: Date; sent: boolean; sentDate?: string; refundProof?: RefundProof };
  ...
  // Credits Applied section (DES-719/763) — rendered just below the invoice details. Recent-first;
  // collapse to 2 with "View all"; each row opens the credit-note detail page (actions live there).
  const creditsAppliedSection = creditNotes.length === 0 ? null : (() => {
    ...
    const collapsible = ordered.length > 2 && !cnExpanded;
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="bg-[#faf9f4] border border-dashed border-[rgba(160,160,160,0.3)] rounded-xl px-4">
          {visible.map(({ cn, idx }, i) => {
            ...
            const onRowTap = () => setViewingCnIndex(idx);
            return (
              <div key={cn.no} className={`py-3 ${rowBorder}`}>
                ...
                {proof && (
                  <div className="mt-2 rounded-lg border border-[rgba(15,157,88,0.25)] bg-[rgba(15,157,88,0.06)] px-2.5 py-2 flex flex-col gap-2">
                    ...
                    {proof.proofFile && (
                      <button
                        onClick={(e) => { e.stopPropagation(); setProofPreview({ name: proof.proofFile!, size: 128000 }); }}
          ...
          {collapsible && (
            <button onClick={() => setCnExpanded(true)} className="w-full flex items-center justify-center gap-1 py-3 border-t border-[rgba(160,160,160,0.18)]">
              <span className="text-[13px] font-medium" style={{ ...FONT, color: INK }}>View all credit notes ({creditNotes.length})</span>
            </button>
          )}
          {cancellable && (
            <button onClick={() => setCreditFormOpen(true)} className="group w-full flex items-center gap-1.5 py-3 border-t border-[rgba(160,160,160,0.18)]">
          ...
          {isRefundContext && !fullyRefunded && outstanding > 0.001 && (
            <button onClick={() => setRefundFormOpen(true)} className="group w-full flex items-center gap-1.5 py-3 border-t border-[rgba(160,160,160,0.18)]">
              <AddIcon style={{ fontSize: 18, color: "#ff4a15" }} />
    ...
    <div className="relative bg-white rounded-[48px] overflow-hidden shadow-2xl flex flex-col" style={{ width: 375, height: 812 }}>
      ...
      <BottomSheet open={actionsOpen} title="Invoice actions" onClose={() => setActionsOpen(false)}>
        <div className="flex flex-col">
          ...
          {status === "Paid" && creditNotes.length === 0 && (
            <button
              onClick={() => { setActionsOpen(false); setRefundFormOpen(true); }}
          ...
          {cancellable && creditNotes.length === 0 && (
            <button
              onClick={() => { setActionsOpen(false); setCreditFormOpen(true); }}
              className="w-full flex items-center gap-3 py-3.5 text-left border-b border-[#f1f1f1]"
            >
              <ReceiptLongOutlinedIcon style={{ fontSize: 20, color: "#b42318" }} />
      ...
      <BottomSheet
        ...
        footer={
          <ButtonDock
            ...
            onSecondary={() => { setResendPromptOpen(false); setLocalToast("Credit note updated"); }}
            onPrimary={() => { setResendPromptOpen(false); setSendContext("creditNote"); setSendSheetOpen(true); }}
            homeIndicator
          />
        }
      >
      ...
      <BottomSheet
        ...
        footer={
          <ButtonDock
            ...
            onPrimary={() => { setSendPickerOpen(false); setSendSheetOpen(true); }}
            homeIndicator
          />
        }
      >
        ...
        <div className="flex flex-col gap-2">
          {creditNotes.map((cn, i) => ({ cn, i })).reverse().map(({ cn, i }) => (
            <Tile
              ...
              showStatus={cn.sent}
              status="SENT"
              selected={sendCnIndex === i}
              onClick={() => setSendCnIndex(i)}
            />
          ))}
        </div>
      </BottomSheet>
