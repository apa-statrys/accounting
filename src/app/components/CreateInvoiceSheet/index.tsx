 ...
 * sheet — its own ✕ (frame/library steps) exits the whole Create flow (never falls back to
 * this chooser); the review step's back chevron only cancels that one shot, not the flow.
 * ScanDocument handles its own capture → review (Use Image) → add-another/finish loop
 * internally; `onUpload` only fires once, on Done, with the total page count — all pages are
 * one invoice.
 */
